...
# COPY mosquitto.conf acl.js mqtt.js mqtt.sh /mosquitto/config/
# EXPOSE 8883
...
COPY docker-entrypoint.sh /
COPY mosquitto.conf ca.crt client.crt client.csr client.key /mqtt/config/
EXPOSE 1883
ENTRYPOINT ["/docker-entrypoint.sh"]
CMD ["/usr/sbin/mqtt", "-c", "/mqtt/config/mosquitto.conf"]